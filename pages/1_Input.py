...
st.title("📥 Feed Configuration & Ingestion")
...
with col1:
    ...
    cable_url = st.text_input(
        "Cable Fault API Base URL",
        value=settings.cable_fault_feed_url,
        key="cable_url_input",
    )
...
with col2:
    ...
    aws_url = st.text_input("AWS Health URL", value=settings.cloud_status_aws_url, key="aws_url")
    azure_url = st.text_input("Azure Status URL", value=settings.cloud_status_azure_url, key="azure_url")
    gcp_url = st.text_input("GCP Incidents URL", value=settings.cloud_status_gcp_url, key="gcp_url")
    ...
    if st.button("▶️ Run Full Ingestion Cycle", type="primary", use_container_width=True):
        with st.spinner("Running ingestion cycle..."):
            try:
                ...
                incidents, metrics = loop.run_until_complete(
                    run_ingestion_cycle(settings)
                )
                loop.close()
    ...
    if st.button("💉 Inject Demo Incidents", use_container_width=True):
        demo_incidents = [
            CableIncidentPayload(
                cable_id="aae-1",
                cable_name="AAE-1",
                fault_type="suspected_sabotage",
                status="cut",
                zone="red_sea_bab_el_mandeb",
                fault_location={"lat": 12.6, "lon": 43.5},
                detected_at=datetime.now(timezone.utc),
                reported_by="demo_injector",
                affected_segment_km=45.0,
                estimated_repair_days=21,
                vessel_correlations=[
                    {
                        "mmsi": "412345678",
                        "vessel_name": "MV Shadow Runner",
                        "flag_state": "UNKNOWN",
                        "distance_to_fault_km": 2.3,
                        "is_flagged_shadow_fleet": True,
                    }
                ],
                raw_source_payload={"source": "demo", "injected": True},
            ),
            CableIncidentPayload(
                cable_id="c-lion1",
                cable_name="C-Lion1",
                fault_type="anchor_drag",
                status="under_repair",
                zone="baltic_sea",
                fault_location={"lat": 58.5, "lon": 20.0},
                detected_at=datetime.now(timezone.utc),
                reported_by="demo_injector",
                affected_segment_km=12.0,
                repair_vessel_assigned="CS Responder",
                estimated_repair_days=14,
                vessel_correlations=[
                    {
                        "mmsi": "273456789",
                        "vessel_name": "MV Baltic Ghost",
                        "flag_state": "CM",
            ...
            CableIncidentPayload(
                cable_id="topaz",
                cable_name="Topaz",
                fault_type="seismic_activity",
                status="degraded",
                zone="taiwan_luzon_strait",
                fault_location={"lat": 22.0, "lon": 121.5},
                detected_at=datetime.now(timezone.utc),
                reported_by="demo_injector",
                affected_segment_km=8.0,
                estimated_repair_days=28,
                raw_source_payload={"source": "demo", "injected": True},
            ),
            CableIncidentPayload(
                cable_id="seacom",
                cable_name="Seacom",
                fault_type="fishing_trawler",
                status="degraded",
                zone="red_sea_bab_el_mandeb",
                fault_location={"lat": 14.5, "lon": 42.0},
                detected_at=datetime.now(timezone.utc),
                reported_by="demo_injector",
                affected_segment_km=5.0,
                estimated_repair_days=18,
                raw_source_payload={"source": "demo", "injected": True},
            ),
            CableIncidentPayload(
                cable_id="equiano",
                cable_name="Equiano",
                fault_type="equipment_failure",
                status="degraded",
                zone="west_africa_coast",
                fault_location={"lat": 6.5, "lon": 3.4},
                detected_at=datetime.now(timezone.utc),
                reported_by="demo_injector",
                affected_segment_km=2.0,
...
if quarantine_files:
    selected_file = st.selectbox(
        "Select quarantined payload",
        options=[f.name for f in quarantine_files],
    )
    if selected_file:
        filepath = quarantine_dir / selected_file
...
with col_a:
    total_incidents = store.connection.execute("SELECT COUNT(*) FROM cable_incidents").fetchone()[0]
    st.metric("Total Incidents (DB)", total_incidents)
with col_b:
    total_metrics = store.connection.execute("SELECT COUNT(*) FROM cloud_latency_metrics").fetchone()[0]
    st.metric("Latency Metrics (DB)", total_metrics)
with col_c:
